...
**数据与过滤**
- `description` 保留原始英文文本，不翻译、不编造；中文摘要由分析 Agent 在后续阶段生成
- Awesome 列表和课程类仓库最容易漏网，检查时优先排查
- 若过滤后不足 15 条，输出实际条数即可，不得凑数

**请求处理**
- 单次请求失败需重试（最多 3 次），全部失败记录日志并返回空结果

**输出规范**
- 文件名和字段必须与规范完全一致，不得使用其他值
- 处理流程：curl 获取原始 JSON → Python 脚本解析/过滤/去重/排序 → Write 输出最终文件；Python 脚本中须用 `encoding='utf-8'` 读写文件，避免 Windows GBK 编码问题
